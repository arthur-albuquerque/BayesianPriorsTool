# File: R/mu_weakly_informative_sd.R
...
#' Its main use is to elicitate weakly informative priors.
#'
#' It takes in account the scale of the distribution. For example, effect sizes such as odds ratio,
#' risk ratio, hazard ratio etc are analysed with the log scale. Here we say these effect sizes
#' are in the "relative" scale. In contrast, effect sizes such as risk difference, mean difference
#' and standardized mean difference are in the "absolute" scale.
#'
#' @param upper_limit Numeric value representing the upper limit of the parameter (must be positive if `scale = "relative"`).
...
#' mu_weakly_informative_sd(upper_limit = 10, scale = "absolute")
#'
#' # Relative scale with default mean (1)
#' mu_weakly_informative_sd(upper_limit = 2, scale = "relative")
#'
#' # Custom mean and probability
#' mu_weakly_informative_sd(upper_limit = 5, scale = "absolute", mean = 2, probability = 0.9)
mu_weakly_informative_sd <- function(upper_limit,
                                     scale = c("absolute", "relative"),
                                     mean = NULL,
                                     probability = 0.95) {
  # Validate inputs
  if (!is.numeric(upper_limit) || !is.numeric(probability)) {
    stop("`upper_limit` and `probability` must be numeric.")
  }
  ...
  if (scale == "relative" && (mean <= 0 || upper_limit <= 0)) {
    stop("For `scale = 'relative'`, `mean` and `upper_limit` must be positive.")
  }
  ...
  sd <- switch(scale,
               "absolute" = (upper_limit - mean) / qnorm(1 - tail_probability),
               "relative" = (log(upper_limit) - log(mean)) / qnorm(1 - tail_probability))
